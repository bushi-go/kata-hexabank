...
public class Deposit extends BankOperation {
    ...
    private Currency proceedToCurrency(Scanner scanner) {
        display.print("Do you wish to change the currency ? Is so please enter its code then hit enter key");
    ...
    private String proceedToConfirm(double amount, Currency currency, String accountNumber,
        ...
        String confirm = "";
        while (doing) {
            display.print("Do you confirm the deposit of " + amount + " " + currency.getSymbol() + " on account " +
                    accountNumber + " ?");
            display.print("enter Y to confirm, enter N to cancel and go back to menu");
            confirm = scanner.nextLine();
            if (!confirm.isBlank() &&
                    (confirm.equalsIgnoreCase("Y") || confirm.equalsIgnoreCase("N"))) {
                doing = false;
            }
        }
        return confirm;
